...
import { deleteUserRow, getSuperAdminId, getUserProjects, listUsers, removeUserProject, setUserProject } from "@/db";
...
export async function usersWithGrants() {
  ...
  return Promise.all(users.map(async (u) => ({ ...u, projects: await getUserProjects(u.id) })));
}
